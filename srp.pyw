...
class Settings:
    def __init__(self, master):
        ...
        self.icon_i = tk.PhotoImage(file='assets/settings.png')
...
class Sdpra:
    def __init__(self, master):
        ...
        self.icon_i = tk.PhotoImage(file='assets/stinky.png')
...
def syncing():
    ...
    while sdpra.still_running():
        ...
        if onetime == True:
            asd = requests.get(f"https://media.retroachievements.org/UserPic/{username}.png")
            pfp = Image.open(io.BytesIO(asd.content))
            npfp = pfp.resize((50, 50), Image.LANCZOS)
            with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as temp:
                temp_path = temp.name
                npfp.save(temp_path)
            
            sdpra.ra_pfp=tk.PhotoImage(file=temp_path)
        ...
        if game_title != game_title_stored:
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as temp:
                temp.write(ngi.content)
                temp_path = temp.name
            sdpra.game_image = tk.PhotoImage(file=temp_path)
